...
def httpEnum(ip_address, port):
    ...
    #NIKTOSCAN = "nikto -host %s -port %s > %s/%s._nikto" % (ip_address, port, ip_address, ip_address)
...
def httpsEnum(ip_address, port):
    ...
    #subprocess.call(DIRBUST, shell=True)
    #NIKTOSCAN = "nikto -host %s -port %s >> %s/%s._nikto" % (ip, pr, ip_address, ip_address)
    #subprocess.call(NIKTOSCAN, shell=True)
    return
...
#	    multProc(smbEnum, ip_address, port)
 	 ...
 	 for port in ports:
	    ...
	    multProc(mssqlEnum, ip_address, port)
         ...
         for port in ports:
            port = port.split("/")[0]
            multProc(unknownEnum, ip_address, port)
